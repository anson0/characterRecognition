...
void combineRect(cv::Rect& rectDest,cv::Rect rectSrc);
...
int main() {
    ...
    imgTrainingNumbers = cv::imread("京.png");          // read in training numbers image
    ...
    cv::adaptiveThreshold(imgBlurred,           // input image
        ...
        cv::THRESH_BINARY_INV,                  // invert so foreground will be white, background will be black，11
        11,                                     // size of a pixel neighborhood used to calculate threshold value
        2);                                     // constant subtracted from the mean or weighted mean

    cv::imshow("imgThresh", imgThresh);

    cv::waitKey(0);         // show threshold image for reference
...
//        if (cv::contourArea(ptContours[i]) > MIN_CONTOUR_AREA/20) {                // if contour is big enough to consider
...
//    }   // end for
    ...
    cv::Rect rectDest;
    bool bFlagRectDestAssigned=false;
    ...
    for (int i = 0; i < ptContours.size(); i++)
    {
        if (cv::contourArea(ptContours[i]) > MIN_CONTOUR_AREA/20)
        {
            ...
            if(bFlagRectDestAssigned==false)
            {
                bFlagRectDestAssigned=true;
                rectDest=boundingRect;
                continue;
            }
            combineRect(rectDest,boundingRect);
        }

    }
    cv::rectangle(imgTrainingNumbers, rectDest, cv::Scalar(0, 0, 255), 2);      // draw red rectangle around each contour as we ask user for input

    cv::Mat matROI = imgThresh(rectDest);           // get ROI image of bounding rect
...
void combineRect(cv::Rect& rectDest,cv::Rect rectSrc)
{
    auto xMin=std::min(rectDest.x,rectSrc.x);
    auto yMin=std::min(rectDest.y,rectSrc.y);
    auto xMax=std::max(rectDest.x+rectDest.width,rectSrc.x+rectSrc.width);
    auto yMax=std::max(rectDest.y+rectDest.height,rectSrc.y+rectSrc.height);
    rectDest.x=xMin;
    rectDest.y=yMin;
    rectDest.width=xMax-xMin;
    rectDest.height=yMax-yMin;
